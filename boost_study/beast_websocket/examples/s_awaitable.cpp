...
net::awaitable<void> do_session(stream ws) {
    ...
    for (;;) try {
            ...
            if (se.code() != websocket::error::closed) throw;
        }
}
...
net::awaitable<void> do_listen(tcp::endpoint endpoint) {
    ...
    cout << "listen, net::socket_base::max_listen_connections:" << net::socket_base::max_listen_connections << endl;

    for (;;)
        boost::asio::co_spawn(acceptor.get_executor(), do_session(stream(co_await acceptor.async_accept())),
                              [](std::exception_ptr e) {
                                  try {
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        std::cerr << "Usage: websocket-server-awaitable <address> <port> <threads>\n"
                  << "Example:\n"
