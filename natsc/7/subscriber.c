...
void onMsg(natsConnection *nc, natsSubscription *sub, natsMsg *msg, void *closure) {
    // printf("onMsg\n");
    auto tid = getThreadTid();

    static std::size_t last_tid = 0;
    if (last_tid == 0) {
        last_tid = tid;
    } else {
        if (last_tid != tid) {
    ...
    if (print) {
        printf("%lu Received subject:%s len:%d \n",
               tid, natsMsg_GetSubject(msg), natsMsg_GetDataLength(msg));
    }
    ...
    if (print) {
        printf("%lu bodyWithoutHeadLen:%d from:%d to:%d body:%s \n",
               tid, bodyLen, (int)nmsg->head.from, (int)nmsg->head.to, recv_buff);
    }
